...
<body>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST" id="admin-verify-form">
                            <div class="mb-3">
                                <label for="payment_id" class="form-label">Payment ID</label>
                                <div class="input-group">
                                    <input type="text" class="form-control" id="payment_id" name="payment_id" required>
                                    <button class="btn btn-outline-secondary" type="button" id="paste-payment-id">Paste from Clipboard</button>
                                </div>
                            </div>
                            <div class="mb-3">
    ...
    <script>
        ...
        document.getElementById('paste-payment-id').onclick = async function() {
            try {
                const text = await navigator.clipboard.readText();
                document.getElementById('payment_id').value = text;
            } catch (e) {
                alert('Could not read clipboard. Please paste manually.');
            }
        };

        document.getElementById('admin-verify-form').onsubmit = function(e) {
            document.getElementById('verify-spinner').style.display = 'inline-block';
        };
    </script>

    <?php if ($message && !empty($booking)): ?>
    <div class="card mt-3">
        ...
        <div class="card-body">
            ...
            <p><strong>Email:</strong> <?php echo htmlspecialchars($booking['contact_email'] ?? ''); ?></p>
            ...
            <p><strong>Status:</strong> <?php echo htmlspecialchars($booking['payment_status'] ?? ''); ?></p>
        </div>
    </div>
    <?php endif; ?>
</body>
